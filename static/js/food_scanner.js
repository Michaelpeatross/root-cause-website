(function () {
  ...
  function renderResult(data) {
    if (!data || !data.ok) {
      showError((data && data.error) || 'Could not score that item.');
      return;
    }
    var p = data.product || {};
    var r = data.rating || {};
    var notes = (r.personal_notes || []).map(function (n) {
      return '<p class="hit">' + escapeHtml(n) + '</p>';
    }).join('');
    var additives = (r.additive_hits || []).length
      ? '<p class="hit">Caution additives: ' + escapeHtml(r.additive_hits.join(', ')) + '</p>'
      : '';
    var img = p.image ? '<img class="prod" src="' + escapeHtml(p.image) + '" alt="">' : '';
    resultEl.hidden = false;
    resultEl.innerHTML =
      '<div class="card">' +
      '<div class="score-ring" style="background:' + escapeHtml(r.color || '#555') + '">' +
      '<div class="num">' + escapeHtml(r.score) + '</div>' +
      '<div class="lbl">' + escapeHtml(r.label || '') + '</div></div>' +
      '<div style="display:flex;gap:1rem;align-items:flex-start;">' + img +
      '<div><h2 style="margin:0 0 .25rem;">' + escapeHtml(p.name || 'Product') + '</h2>' +
      '<p style="color:var(--text-muted);margin:0;">' + escapeHtml(p.brands || '') +
      (p.code ? ' · ' + escapeHtml(p.code) : '') + '</p></div></div>' +
      '<div style="margin-top:1rem;">' +
      row('Nutrition quality', r.nutrition_score) +
      row('Additives / processing', r.additive_score) +
      row('Fit for your scan', r.personal_score) +
      '</div>' + notes + additives +
      (p.ingredients ? '<p style="font-size:.85rem;margin-top:1rem;"><strong>Ingredients</strong><br>' +
        escapeHtml(p.ingredients).slice(0, 600) + '</p>' : '') +
      '</div>';
    resultEl.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }
  function row(label, value) {
    return '<div class="break-row"><span>' + escapeHtml(label) + '</span><strong>' +
      escapeHtml(value) + '/100</strong></div>';
  }
  ...
  function scoreBarcode(code) {
    ...
    postJSON('/api/food-scan/barcode', { barcode: code })
      .then(renderResult)
      .catch(function () { showError('Network error scoring that barcode.'); });
  }
  ...
  document.getElementById('score-photo').addEventListener('click', function () {
    ...
    reader.onload = function () {
      ...
      postJSON('/api/food-scan/photo', { image_b64: b64, mime: mime })
        .then(renderResult)
        .catch(function () { showError('Could not read that photo.'); });
    };
  ...
  document.getElementById('search-form').addEventListener('submit', function (evt) {
    ...
    postJSON('/api/food-scan/search', { q: q }).then(function (data) {
      ...
      if (!items.length) {
        box.innerHTML = '<p>No matches. Try the barcode or a label photo.</p>';
        return;
      }
      box.innerHTML = items.map(function (item) {
        return '<div class="search-hit" data-code="' + escapeHtml(item.code) + '">' +
          '<div><strong>' + escapeHtml(item.name) + '</strong><div style="color:var(--text-muted);font-size:.85rem;">' +
          escapeHtml(item.brands || item.code) + '</div></div>' +
          '<span>' + escapeHtml(item.nutriscore || '') + '</span></div>';
      }).join('');
      box.querySelectorAll('.search-hit').forEach(function (rowEl) {
        rowEl.addEventListener('click', function () {
          scoreBarcode(rowEl.getAttribute('data-code'));
        });
      });
    }).catch(function () {
      box.innerHTML = '<p>Search failed. Try a barcode.</p>';
    });
  });
  function stopCam() {
    scanning = false;
  ...
  document.getElementById('start-cam').addEventListener('click', function () {
    ...
    navigator.mediaDevices.getUserMedia({ video: { facingMode: { ideal: 'environment' } }, audio: false }).then(function (media) {
      ...
      scanning = true;
      if (camStatus) {
        camStatus.textContent = ('BarcodeDetector' in window)
          ? 'Point at the barcode…'
          : 'Live barcode reading is not in this browser. Snap a photo of the barcode instead.';
      }
      detectLoop();
    }).catch(function () {
