...
from health_advisor import (
    ...
    test_grok_connection, generate_wearable_summary,
)
...
def dashboard():
    ...
    if request.method == 'POST':
        ...
        if action == 'update_phone':
            ...
            try:
                file_list = request.files.getlist('health_files')
                if not any(f and f.filename for f in file_list):
                    flash('Please select health data export files (Apple Health export.xml/zip, device CSVs, etc.).', 'error')
                else:
                    upload_result = save_multiple_uploads(file_list, documents_dir)
                    if isinstance(upload_result, tuple):
                        saved_files, partial_errors = upload_result
