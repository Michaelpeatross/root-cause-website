...
TAB_CSS = """
.rc-tablist{display:flex;flex-wrap:wrap;gap:.4rem;margin:0 0 1rem;padding:.15rem 0 .25rem;position:sticky;top:0;z-index:5;background:linear-gradient(#fff 70%,rgba(255,255,255,.92));}
...
def _questions_panel(html):
    ...
    return ('<section class="rc-tab-panel" data-panel="questions" id="tab-questions">'
            ...
            '<ol class="rc-q-list">' + lis + '</ol></section>')


def _ask_panel(html):
    titles = _extract_priority_titles(html)
    chips = [
        ('How do I read this wellness report?', 'How do I read this wellness report in plain English?'),
    ...
    return ('<section class="rc-tab-panel" data-panel="ask" id="tab-ask"><div class="rc-ask-card">'
            '<h2>Ask Grok about this report</h2>'
            '<p>Grok can explain a section in everyday language. Keep questions about this report. Answers are educational only - not medical advice.</p>'
            '<div class="rc-ask-chips">' + chip_html + '</div>'
            '<button type="button" class="rc-ask-open" data-ask="Help me understand my top priorities on this wellness report.">Open Ask Grok</button>'
            '</div></section>')
...
def _split_inner(inner):
    ...
    pattern = re.compile(
        r'(<aside class="wellness-banner"[\\s\\S]*?</aside>)'
        r'|(<details class="glossary-panel"[\\s\\S]*?</details>)'
        r'|(<section class="top3"[\\s\\S]*?</section>)'
        r'|(<section[^>]*id="body-overview"[\\s\\S]*?</section>)'
        r'|(<section class="scan-section[\\s\\S]*?</section>)'
        r'|(<header class="scan-cover"[\\s\\S]*?</header>)'
        r'|(<footer class="scan-disclaimer"[\\s\\S]*?</footer>)',
        re.I,
    )
    pos = 0
    leftover = []
...
def apply_report_tabs(html):
    ...
    tablist = '<div class="rc-tablist" role="tablist" aria-label="Report sections">' + ''.join(
        '<button type="button" class="rc-tab%s" role="tab" data-tab="%s" aria-controls="tab-%s" aria-selected="%s">%s</button>'
        % (' is-active' if i == 0 else '', key, key, 'true' if i == 0 else 'false', escape(label))
        for i, (key, label) in enumerate(buttons)
    ) + '</div>'
    match = re.search(r'(<div class="wellness-report"[^>]*>)([\\s\\S]*)</div>\\s*$', html)
    if match:
        ...
        for key in ('overview', 'systems', 'food', 'supplements'):
            ...
            panels.append('<section class="rc-tab-panel%s" data-panel="%s" id="tab-%s" role="tabpanel">%s</section>' % (active, key, key, body))
